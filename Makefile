...
all := $(target)_x86_64.exe $(target)_arm64.exe $(target)_darwin_x86_64 $(target)_darwin_arm64 $(target)_linux_x86_64 $(target)_linux_arm64
...
# Windows
$(target)_x86_64.exe: $(src)
	GOOS=windows GOARCH=amd64 go build -ldflags "-s" -o $(target)_x86_64.exe

$(target)_arm64.exe: $(src)
	GOOS=windows GOARCH=arm64 go build -ldflags "-s" -o $(target)_arm64.exe
